...
namespace Sharpheus {
	...
	void ClassWriter::CreateBehaviorHeader(const wxString& filePath, const wxString& className, const wxString& parent, uint32_t subType)
	{
	...
	void ClassWriter::UpdateCreateBehaviorHeader(const wxString& filePath, const wxString& newClassPath, const wxString& newClassName, uint32_t newSubType, std::unordered_map<uint32_t, std::string>& oldClasses)
	{
		...
		cw.StartClass();
		cw.WriteVisibility("public");
		cw.WriteFunctionDecl("Sharpheus::Behavior*", "Create", "uint32_t subType, Sharpheus::PlaceholderBehavior* other", "static");
		cw.WriteFunctionDecl("Sharpheus::Behavior*", "Create", "uint32_t subType, Sharpheus::GameObject* parent, const std::string& name", "static");
		cw.WriteEmptyLine();
		cw.WriteFunctionDecl("bool", "IsCompatibleWithParent", "uint32_t subType, Sharpheus::GameObject* parent", "static");
		cw.WriteEmptyLine();
		cw.WriteLine(1, "static std::unordered_map<uint32_t, std::string> behaviorNames;");
		cw.EndClass();
		cw.WriteEmptyLine();
	}


	void ClassWriter::UpdateCreateBehaviorSource(const wxString& filePath, const wxString& newClassName, uint32_t newSubType, std::unordered_map<uint32_t, std::string>& oldClasses)
	{
		...
		cw.WriteLine("std::unordered_map<uint32_t, std::string> BehaviorCreator::behaviorNames({");
		for (auto it = oldClasses.begin(); it != oldClasses.end(); ++it) {
		...
		cw.StartFunctionDef("Sharpheus::Behavior*", "Create", "uint32_t subType, Sharpheus::PlaceholderBehavior* other");
		...
		cw.StartFunctionDef("Sharpheus::Behavior*", "Create", "uint32_t subType, Sharpheus::GameObject* parent, const std::string& name");
		cw.WriteLine(1, "SPH_START_BEHAVIORS(subType)");
		...
		cw.StartFunctionDef("bool", "IsCompatibleWithParent", "uint32_t subType, Sharpheus::GameObject* parent");
		cw.WriteLine(1, "SPH_START_BEHAVIORS(subType)");
	...
	void ClassWriter::WriteLine(uint32_t tabs, const wxString& line)
	{
	...
	void ClassWriter::WriteEmptyLines(uint32_t number)
	{
